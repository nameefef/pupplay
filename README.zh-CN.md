...
- **速度 5 档、数量 1–10 只、大小 10 档** 可调。大小 10 档由屏幕推导，两端各有硬约束：下限 18mm（按犬类视敏度算出的「能看清形状」的最小尺寸），上限为屏幕短边的 45%（再大就没地方跑，追不起来）。手机上是 18~32mm，平板自动拉宽到 18~63mm
...
许可证覆盖整个项目历史。但只有 **v1.4 及以后的版本以二进制形式分发**——更早的 Release 已经撤下,因为那些 APK 是在许可证被打包进安装包之前构建的。见 [CHANGELOG.md](CHANGELOG.md)。
